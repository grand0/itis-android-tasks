...
class FilmCardViewHolder(
    ...
    init {
        with (binding) {
            ...
            ivFavorite.setOnClickListener {
                film?.let { onFavClicked(it, adapterPosition) }
            }
        }
    }
    ...
    fun bindItem(film: FilmModel) {
        ...
        with (binding) {
            ...
            ivFavorite.setImageResource(
                if (film.isFavorite) R.drawable.baseline_favorite_24 else R.drawable.baseline_favorite_border_24
            )
            glide
                .load(film.imageUrl)
    ...
    fun setColorForInfo(@ColorInt color: Int) {
        with (binding) {
            ...
            ivFavorite.imageTintList = ColorStateList.valueOf(color)
